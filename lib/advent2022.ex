defmodule Advent2022Day1 do
  ...
  def run do
    calory_counts = File.cwd() |> open() |> read_file() |> splitify() |> inventories_to_number() |> sum_inventories()
    largest_calory = Enum.max(calory_counts)
    top_three = Enum.sort(calory_counts, fn l, r -> l >= r end) |> Enum.slice(0..2) |> List.foldl(0, fn e,a -> a + e end)
    IO.puts("Biggest caloric contributor: " <> Integer.to_string(largest_calory))
    IO.puts("Top three total: " <> Integer.to_string(top_three))
    :ok
  end

  def splitify(calory_counts)
  do
    Enum.chunk_by(calory_counts, fn e -> e == "" end) |> Enum.reject(fn [head | _tail] -> head == "" end)
  end

  def to_number(calory_counts)
  do
    for calory <- calory_counts, do: Integer.parse(calory, 10) |> elem(0)
  end

  def inventories_to_number(inventories)
  do
    for caloric_inventory <- inventories, do: to_number(caloric_inventory)
  end

  def sum_inventories(inventories)
  do
