defmodule Advent2022day3 do
  ...
  def run do
    ...
    badges = for triple <- triples, do: find_triplicate(Enum.at(triple, 0), Enum.at(triple, 1), Enum.at(triple, 2))
  ...
  def find_triplicate(first, second, third)
  do
    first_set = MapSet.new(String.graphemes(first))
    second_set = MapSet.new(String.graphemes(second))
    Enum.find(String.graphemes(third), fn char -> MapSet.member?(first_set, char) && MapSet.member?(second_set, char) end)
  end


  def find_priority(to_score)
  do
    for item <- to_score, do: match_priority(item)
  end

  defp match_priority(nil)
  do
    0
  end

  defp match_priority(item)
  do
    <<ascii::utf8>> = item
    cond do
      ascii < 96 ->
